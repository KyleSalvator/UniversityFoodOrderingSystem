...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['order_id']) && isset($_POST['paid'])) {
    ...
    // Update the Paid_By field in the database
    $updateSql = "UPDATE to_deliver SET Paid_By='$paidBy' WHERE order_id=$orderId";
...
// Query to fetch unpaid orders
$sql = "SELECT * FROM to_deliver WHERE Paid_By IS NULL OR Paid_By = ''"; // Filter only orders that are not paid
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<tr id='order_" . $row["order_id"] . "'>"; // Add id to identify the row
        ...
        echo "<td><form method='post'><input type='hidden' name='order_id' value='" . $row["order_id"] . "'><button type='submit' name='paid' value='Paid' class='btn btn-success'>Paid</button></form></td>"; // Add form to submit data to update Paid_By
        echo "</tr>";
    }
    echo "</tbody>";
    echo "</table>";
