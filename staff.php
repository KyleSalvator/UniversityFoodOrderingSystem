...
<head>
    ...
    <style>
        body {
            ...
            background-repeat: no-repeat; /* Do not repeat the image */   background-image: url('assets/images/mbr-1920x1276.jpg'); /* Set background image */
        background-size: cover; /* Cover the entire screen */
        ...
        <nav class="navbar navbar-dropdown navbar-fixed-top navbar-expand-lg">
            <div class="container">
                <div class="navbar-brand">
                    <span class="navbar-caption-wrap"><a href="gg.php" class="navbar-caption text-black display-4">UFO </a></span>
                </div>
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav nav-dropdown nav-right" data-app-modern-menu="true">
                        <li class="nav-item">
                            <a class="nav-link link text-black display-4" href="staff.php">Products</a>
                        </li>
                        <li class="nav-item">
                        ...
                        <li class="nav-item">
                            <a class="nav-link link text-black display-4" id="proceedToPaymentBtn"href="todeliver2.php">To Deliver</a>
                        </li>
                        <li>
                            <strong>
                                <button>
...
if(isset($_POST['update_status'])){
    $productId = $_POST['product_id'];
    $status = $_POST['update_status']; // Updated this line to get the correct value
...
if ($result->num_rows > 0) {
    ...
    echo "<table>";
    echo "<thead><tr><th>Product ID</th><th>Product Name</th><th>Description</th><th>Price</th><th>Image</th><th>Status</th><th>Action</th></tr></thead>";
    echo "<tbody>";
    ...
    while($row = $result->fetch_assoc()) {
        ...
        <form method='post' action=''>
            ...
            <button type='submit' class='btn " . ($row["status"] == 'AVAILABLE' ? 'btn-success' : 'btn-danger') . "' name='update_status' value='" . ($row["status"] == 'AVAILABLE' ? 'UNAVAILABLE' : 'AVAILABLE') . "' style='width: 120px;'>" . ($row["status"] == 'AVAILABLE' ? 'Available' : 'Unavailable') . "</button>
        </form>
      </td>";
        echo "<td>
                ...
                <form method='post' action=''>
                    <input type='hidden' name='product_id' value='" . $row["product_id"] . "'>
                    <button type='submit' class='btn remove-btn' name='delete'>Remove</button>
                </form>
              </td>";
        echo "</tr>";
    }
...
?>
        ...
        <form>
            <center><a href="3p.php" class="btn btn-primary add-menu-btn" name="submit">Add Menu</a></center>
        </form>
    </div>
        ...
        function saveRow(button) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    throw new Error('save changes');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                alert('Saved changes. Please Refresh page.');
            });
        }
    </script>
